...
public class LevelChangeManager : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        for (int i = 0; i < levelButton.Count; i++)
        {
            ...
            {
                case State.Locked: levelButton[i].transform.GetChild(0).GetComponent<Image>().sprite = StLocked;
                    levelButton[i].GetComponent<Image>().sprite = LvLocked;
                    break;


                case State.Unlocked:
                    levelButton[i].transform.GetChild(0).GetComponent<Image>().sprite = StUnlocked;
